...
- `import-normalizer.js` には、各種教材フォーマットをアプリのスキーマに合わせて整形する関数群をまとめています。
- ブラウザでは `index.html` で自動読み込みされ、インポート時に利用されます。
- Node.js からも `require('./import-normalizer')` で利用でき、`normalizeQuestions(raw)` などを呼び出すと `{ valid, invalidReasons }` を返します。サーバサイドで取り込む際も同じロジックを再利用できます。
- 科目・年度・番号・難易度が未指定の場合でも、正規化時に `未分類` / `不明` として補完されるため、最低限の設問本文と選択肢が揃っていれば取り込めます。
